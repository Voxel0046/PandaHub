package dev.panda.hub.providers.tab.layout;
...
import dev.panda.hub.providers.tab.util.Reflection;
...
public class TabLayout {
    ...
    @Getter
    private static final Map<UUID, TabLayout> layoutMapping = new ConcurrentHashMap<>();
    ...
    public TabLayout(Tablist instance, Player player) {
    ...
    public void setHeaderAndFooter() {
        boolean continueAt = true; // force true
        ...
        if (continueAt) {
            ...
            IChatBaseComponent headerComponent = IChatBaseComponent.ChatSerializer.a("{text:\"" + StringEscapeUtils.escapeJava(header) + "\"}");
            IChatBaseComponent footerComponent = IChatBaseComponent.ChatSerializer.a("{text:\"" + StringEscapeUtils.escapeJava(footer) + "\"}");
    ...
    private void fetchPing(int index, GameProfile gameProfile, int ping) {
        ...
        PacketPlayOutPlayerInfo packetPlayOutPlayerInfo = new PacketPlayOutPlayerInfo(PacketPlayOutPlayerInfo.EnumPlayerInfoAction.UPDATE_LATENCY, entityPlayer);
    ...
    public void create() {
        ...
        List<TabEntry> initialEntries = instance.getAdapter().getLines(player);

        for (int row = 0; row < 20; row++) {
            for (int column = 0; column < 3; column++) {
                int index = row + column * 20;

                Skin initialSkin = getInitialSkin(initialEntries, index);
                skinMapping.put(index, initialSkin);
...
//		Bukkit.getLogger().info("Send info datas");
    ...
    private Skin getInitialSkin(List<TabEntry> entries, int index) {
        int row = index % 20;
        int column = index / 20;

        for (TabEntry entry : entries) {
            if (entry.getColumn() == column && entry.getRow() == row) {
                return entry.getSkin() != null ? entry.getSkin() : Skin.DEFAULT_SKIN;
            }
        }
