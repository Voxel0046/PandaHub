...
import dev.panda.hub.commons.cosmetics.outfit.menu.buttons.chestplate.OutfitChestplateButton;
...
public class OutfitChestplateMenu extends PaginatedMenu {

    private OutfitManager outfitManager;

    public OutfitChestplateMenu() {
        this.outfitManager = PandaHub.get().getModuleService().getManagerModule().getOutfitManager();
    }

    @Override
    public String getPrePaginatedTitle(Player player) {
        return ChatUtil.translate(CosmeticService.OUTFIT_CHESTPLATE_TITLE);
    }
    ...
    public Map<Integer, Button> getAllPagesButtons(Player player) {
        ...
        for (Outfit outfit : outfitManager.getOutfits().values()) {
            ...
            buttons.put(validSlots.get(index), new OutfitChestplateButton(outfit));
            index++;
        }
