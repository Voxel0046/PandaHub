...
import dev.panda.hub.commons.cosmetics.outfit.menu.buttons.helmet.OutfitHelmetButton;
...
public class OutfitHelmetMenu extends PaginatedMenu {

    private OutfitManager outfitManager;

    public OutfitHelmetMenu() {
        this.outfitManager = PandaHub.get().getModuleService().getManagerModule().getOutfitManager();
    }

    @Override
    public String getPrePaginatedTitle(Player player) {
        return ChatUtil.translate(CosmeticService.OUTFIT_HELMET_TITLE);
    }
    ...
    public Map<Integer, Button> getAllPagesButtons(Player player) {
        ...
        for (Outfit outfit : outfitManager.getOutfits().values()) {
            ...
            buttons.put(validSlots.get(index), new OutfitHelmetButton(outfit));
            index++;
        }
